/**
 * This file was auto-generated by Fern from our API Definition.
 */
package com.paid.api.types;

import com.fasterxml.jackson.annotation.JsonAnyGetter;
import com.fasterxml.jackson.annotation.JsonAnySetter;
import com.fasterxml.jackson.annotation.JsonIgnoreProperties;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonSetter;
import com.fasterxml.jackson.annotation.Nulls;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.paid.api.core.ObjectMappers;
import java.util.HashMap;
import java.util.Map;
import java.util.Objects;
import java.util.Optional;

@JsonInclude(JsonInclude.Include.NON_ABSENT)
@JsonDeserialize(builder = ProductTieredPricePointTiersItem.Builder.class)
public final class ProductTieredPricePointTiersItem {
    private final Optional<Double> flatAmount;

    private final Optional<Double> index;

    private final double lowerBound;

    private final Optional<Double> number;

    private final Optional<ProductTieredPricePointTiersItemTierBillingType> tierBillingType;

    private final Optional<Double> unitAmount;

    private final Optional<Double> upperBound;

    private final Map<String, Object> additionalProperties;

    private ProductTieredPricePointTiersItem(
            Optional<Double> flatAmount,
            Optional<Double> index,
            double lowerBound,
            Optional<Double> number,
            Optional<ProductTieredPricePointTiersItemTierBillingType> tierBillingType,
            Optional<Double> unitAmount,
            Optional<Double> upperBound,
            Map<String, Object> additionalProperties) {
        this.flatAmount = flatAmount;
        this.index = index;
        this.lowerBound = lowerBound;
        this.number = number;
        this.tierBillingType = tierBillingType;
        this.unitAmount = unitAmount;
        this.upperBound = upperBound;
        this.additionalProperties = additionalProperties;
    }

    @JsonProperty("flatAmount")
    public Optional<Double> getFlatAmount() {
        return flatAmount;
    }

    @JsonProperty("index")
    public Optional<Double> getIndex() {
        return index;
    }

    @JsonProperty("lowerBound")
    public double getLowerBound() {
        return lowerBound;
    }

    @JsonProperty("number")
    public Optional<Double> getNumber() {
        return number;
    }

    @JsonProperty("tierBillingType")
    public Optional<ProductTieredPricePointTiersItemTierBillingType> getTierBillingType() {
        return tierBillingType;
    }

    @JsonProperty("unitAmount")
    public Optional<Double> getUnitAmount() {
        return unitAmount;
    }

    @JsonProperty("upperBound")
    public Optional<Double> getUpperBound() {
        return upperBound;
    }

    @java.lang.Override
    public boolean equals(Object other) {
        if (this == other) return true;
        return other instanceof ProductTieredPricePointTiersItem && equalTo((ProductTieredPricePointTiersItem) other);
    }

    @JsonAnyGetter
    public Map<String, Object> getAdditionalProperties() {
        return this.additionalProperties;
    }

    private boolean equalTo(ProductTieredPricePointTiersItem other) {
        return flatAmount.equals(other.flatAmount)
                && index.equals(other.index)
                && lowerBound == other.lowerBound
                && number.equals(other.number)
                && tierBillingType.equals(other.tierBillingType)
                && unitAmount.equals(other.unitAmount)
                && upperBound.equals(other.upperBound);
    }

    @java.lang.Override
    public int hashCode() {
        return Objects.hash(
                this.flatAmount,
                this.index,
                this.lowerBound,
                this.number,
                this.tierBillingType,
                this.unitAmount,
                this.upperBound);
    }

    @java.lang.Override
    public String toString() {
        return ObjectMappers.stringify(this);
    }

    public static LowerBoundStage builder() {
        return new Builder();
    }

    public interface LowerBoundStage {
        _FinalStage lowerBound(double lowerBound);

        Builder from(ProductTieredPricePointTiersItem other);
    }

    public interface _FinalStage {
        ProductTieredPricePointTiersItem build();

        _FinalStage flatAmount(Optional<Double> flatAmount);

        _FinalStage flatAmount(Double flatAmount);

        _FinalStage index(Optional<Double> index);

        _FinalStage index(Double index);

        _FinalStage number(Optional<Double> number);

        _FinalStage number(Double number);

        _FinalStage tierBillingType(Optional<ProductTieredPricePointTiersItemTierBillingType> tierBillingType);

        _FinalStage tierBillingType(ProductTieredPricePointTiersItemTierBillingType tierBillingType);

        _FinalStage unitAmount(Optional<Double> unitAmount);

        _FinalStage unitAmount(Double unitAmount);

        _FinalStage upperBound(Optional<Double> upperBound);

        _FinalStage upperBound(Double upperBound);
    }

    @JsonIgnoreProperties(ignoreUnknown = true)
    public static final class Builder implements LowerBoundStage, _FinalStage {
        private double lowerBound;

        private Optional<Double> upperBound = Optional.empty();

        private Optional<Double> unitAmount = Optional.empty();

        private Optional<ProductTieredPricePointTiersItemTierBillingType> tierBillingType = Optional.empty();

        private Optional<Double> number = Optional.empty();

        private Optional<Double> index = Optional.empty();

        private Optional<Double> flatAmount = Optional.empty();

        @JsonAnySetter
        private Map<String, Object> additionalProperties = new HashMap<>();

        private Builder() {}

        @java.lang.Override
        public Builder from(ProductTieredPricePointTiersItem other) {
            flatAmount(other.getFlatAmount());
            index(other.getIndex());
            lowerBound(other.getLowerBound());
            number(other.getNumber());
            tierBillingType(other.getTierBillingType());
            unitAmount(other.getUnitAmount());
            upperBound(other.getUpperBound());
            return this;
        }

        @java.lang.Override
        @JsonSetter("lowerBound")
        public _FinalStage lowerBound(double lowerBound) {
            this.lowerBound = lowerBound;
            return this;
        }

        @java.lang.Override
        public _FinalStage upperBound(Double upperBound) {
            this.upperBound = Optional.ofNullable(upperBound);
            return this;
        }

        @java.lang.Override
        @JsonSetter(value = "upperBound", nulls = Nulls.SKIP)
        public _FinalStage upperBound(Optional<Double> upperBound) {
            this.upperBound = upperBound;
            return this;
        }

        @java.lang.Override
        public _FinalStage unitAmount(Double unitAmount) {
            this.unitAmount = Optional.ofNullable(unitAmount);
            return this;
        }

        @java.lang.Override
        @JsonSetter(value = "unitAmount", nulls = Nulls.SKIP)
        public _FinalStage unitAmount(Optional<Double> unitAmount) {
            this.unitAmount = unitAmount;
            return this;
        }

        @java.lang.Override
        public _FinalStage tierBillingType(ProductTieredPricePointTiersItemTierBillingType tierBillingType) {
            this.tierBillingType = Optional.ofNullable(tierBillingType);
            return this;
        }

        @java.lang.Override
        @JsonSetter(value = "tierBillingType", nulls = Nulls.SKIP)
        public _FinalStage tierBillingType(Optional<ProductTieredPricePointTiersItemTierBillingType> tierBillingType) {
            this.tierBillingType = tierBillingType;
            return this;
        }

        @java.lang.Override
        public _FinalStage number(Double number) {
            this.number = Optional.ofNullable(number);
            return this;
        }

        @java.lang.Override
        @JsonSetter(value = "number", nulls = Nulls.SKIP)
        public _FinalStage number(Optional<Double> number) {
            this.number = number;
            return this;
        }

        @java.lang.Override
        public _FinalStage index(Double index) {
            this.index = Optional.ofNullable(index);
            return this;
        }

        @java.lang.Override
        @JsonSetter(value = "index", nulls = Nulls.SKIP)
        public _FinalStage index(Optional<Double> index) {
            this.index = index;
            return this;
        }

        @java.lang.Override
        public _FinalStage flatAmount(Double flatAmount) {
            this.flatAmount = Optional.ofNullable(flatAmount);
            return this;
        }

        @java.lang.Override
        @JsonSetter(value = "flatAmount", nulls = Nulls.SKIP)
        public _FinalStage flatAmount(Optional<Double> flatAmount) {
            this.flatAmount = flatAmount;
            return this;
        }

        @java.lang.Override
        public ProductTieredPricePointTiersItem build() {
            return new ProductTieredPricePointTiersItem(
                    flatAmount,
                    index,
                    lowerBound,
                    number,
                    tierBillingType,
                    unitAmount,
                    upperBound,
                    additionalProperties);
        }
    }
}
